...
parser.add_argument(
    "--compile-mode", type=str, default="",
    choices=["", "default", "reduce-overhead", "max-autotune", "max-autotune-no-cudagraphs"],
    help="optional torch.compile mode; useful for fixed-shape speedruns",
)
# FP8 training
parser.add_argument("--fp8", action="store_true", help="enable FP8 training (requires H100+ GPU)")
parser.add_argument("--fp8-recipe", type=str, default="tensorwise", choices=["rowwise", "tensorwise"], help="FP8 scaling recipe: tensorwise (faster, recommended) or rowwise (more accurate but slower)")
parser.add_argument("--liger-no-sync", action="store_true", help="experimental no-padding Liger adapter; requires --liger-cross-entropy and CPU token validation")
...
parser.add_argument("--mlp-hidden-dim", type=int, default=0, help="absolute MLP hidden width for the trained model (0 = 4 * model_dim); d12 scaling reference stays unchanged")
...
if args.mlp_hidden_dim < 0:
    parser.error("--mlp-hidden-dim must be nonnegative (0 = 4 * model_dim)")
user_config = vars(args).copy()  # for logging
# -----------------------------------------------------------------------------
# Compute init and wandb logging

device_type = autodetect_device_type() if args.device_type == "" else args.device_type
if args.liger_no_sync and not args.liger_cross_entropy:
    parser.error("--liger-no-sync requires --liger-cross-entropy")
...
def build_model_meta(depth, *, mlp_hidden_dim=0):
    """Build shapes only; explicit target overrides never alter the d12 reference."""
...
model = build_model_meta(args.depth, mlp_hidden_dim=args.mlp_hidden_dim) # 1) Build on meta device (only shapes/dtypes, no data)
...
if args.liger_cross_entropy:
    ...
    if args.liger_no_sync:
        from nanochat.liger_nosync import LigerNoSyncCrossEntropyLoss
        model.set_training_loss(LigerNoSyncCrossEntropyLoss(
            vocab_size, targets_validated_on_cpu=True,
        ))
        print0("✓ Experimental no-sync Liger CE enabled; CPU packed-token validation required")
    else:
...
if args.compile_mode:
    ...
    print0(f"Using torch.compile mode: {args.compile_mode}")
...
train_loader = tokenizing_distributed_data_loader_with_state_bos_bestfit(
    ...
    validate_no_padding_vocab_size=vocab_size if args.liger_no_sync else None,
)
