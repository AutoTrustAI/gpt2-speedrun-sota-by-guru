...
class GPTConfig:
    ...
    mlp_hidden_dim: int = 0 # 0 keeps the original 4 * n_embd expansion

    def __post_init__(self):
        if type(self.mlp_hidden_dim) is not int or self.mlp_hidden_dim < 0:
            raise ValueError("mlp_hidden_dim must be a nonnegative integer (0 = 4 * n_embd)")
...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        hidden_dim = config.mlp_hidden_dim or 4 * config.n_embd
